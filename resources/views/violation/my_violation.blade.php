...
@section('content')
    <div class="min-h-screen bg-gray-50/30 p-4 lg:p-10">
        <div class="max-w-6xl mx-auto">
            <header class="mb-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Incident Reports</h1>
                        <p class="text-gray-500 mt-1">Manage and review driver conduct reports and official violations.</p>
                    </div>
                    <div class="flex gap-4">
                        <a href="{{ route('createNewReport') }}"
            ...
            <div class="mb-6">
                <form action="{{ route('viewReport') }}" method="GET" class="flex gap-2">
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400">Incident
                                    Date</th>
                            ...
                            @forelse($reports as $report)
                                <tr class="hover:bg-gray-50/30 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            @if ($report->evidence_image_path)
                                                <img src="{{ Cloudinary::getImage($report->evidence_image_path)->delivery('q_auto')->delivery('f_auto')->resize('thumb', 100, 100, ['gravity' => 'auto'])->toUrl() }}"
                                            ...
                                            <div>
                                                ...
                                                <div class="text-sm font-bold text-gray-900 line-clamp-1">
                                                    {{ $report->driver->username ?? 'Unknown Driver' }}
                                                </div>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @endphp
                                        <span
                                            class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $statusColors[$report->status] ?? 'bg-gray-100' }}">
                                            {{ $report->status }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-700">
                                            {{ \Carbon\Carbon::parse($report->event_date)->format('M d, Y') }}</div>
                                        <div class="text-xs text-gray-400">
                                            {{ \Carbon\Carbon::parse($report->event_date)->format('h:i A') }}</div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center gap-4">
                                            <a href="{{ route('showReport', $report->id) }}"
                                                class="text-blue-600 font-bold hover:underline">Review Details</a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">Clean slate! No violations currently on
                                                record.</p>
                                        </div>
                                    </td>
                                </tr>
