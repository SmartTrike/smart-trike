        ...
        <div class="max-w-5xl mx-auto">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-sm text-amber-700">
                                Please surrender the found item to the Main Terminal Office as soon as possible. Staff will
                                keep the item safe until the owner claims it.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-1 uppercase text-xs font-bold tracking-widest">Case ID: #{{ $item->id }}
                    </p>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    @endphp
                    <span
                        class="px-4 py-1.5 rounded-full border text-xs font-bold uppercase tracking-widest {{ $statusColors[$item->status] ?? 'bg-gray-100 text-gray-700' }}">
                        {{ $item->status }}
                    </span>
                </div>
            </header>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden p-2">
                        @if ($item->image_path)
                            ...
                            <x-cloudinary::image width="100" height="100" crop="thumb"
                                public-id="{{ $item->image_path }}" class="w-full h-80 object-cover rounded-xl shadow-inner"
                                alt="Lost and Found"></x-cloudinary::image>

                            <a href="{{ Storage::disk('cloudinary')->url($item->image_path) }}" target="_blank"
                        ...
                        @else
                            <div
                                class="w-full h-80 bg-gray-100 rounded-xl flex flex-col items-center justify-center text-gray-400">
                    ...
                    @if (Auth::user()->isAdmin() || Auth::user()->isDispatcher())
                        <div class="bg-white border border-gray-900 shadow-xl rounded-2xl p-6">
                            ...
                            <form action="{{ route('updateLostAndFoundStatus', $item->id) }}" method="POST"
                                class="space-y-4">
                                @csrf
                                @method('PATCH')

                                <div>
                                    <label class="block text-2xs font-bold text-gray-400 uppercase mb-1">Change
                                        Status</label>
                                    <select name="status"
                                        class="w-full border-gray-200 rounded-xl text-sm focus:ring-gray-900">
                                        <option value="reported" {{ $item->status == 'reported' ? 'selected' : '' }}>
                                            Reported</option>
                                ...
                                <div>
                                    <label class="block text-2xs font-bold text-gray-400 uppercase mb-1">Internal
                                        Remarks</label>
                                    <textarea name="remarks" rows="2" class="w-full border-gray-200 rounded-xl text-sm focus:ring-gray-900"
                                        placeholder="Notes for staff...">{{ $item->remarks }}</textarea>
                                </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-8 h-full">
                        ...
                        <div class="border-b border-gray-100 pb-4 mb-6">
                            <span class="text-xs font-bold text-blue-600 uppercase tracking-widest">{{ $item->type }}
                                Report</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                            <div>
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Reported
                                    By</label>
                                <div class="flex items-center gap-2">
                                    <p class="text-gray-900 font-semibold">
                                        {{ $item->reporter->username ?? 'Unknown User' }}</p>
                                </div>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Date
                                    Reported</label>
                                <p class="text-gray-900 font-semibold">{{ $item->created_at->format('F d, Y') }}</p>
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Location
                                    Found/Lost</label>
                                <p class="text-gray-900 font-medium">{{ $item->location_found_lost ?? 'Not specified' }}
                                </p>
                            </div>

                            <div class="md:col-span-2 bg-gray-50 rounded-xl p-5">
                                <label
                                    class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Description</label>
                                <p class="text-gray-700 leading-relaxed">{{ $item->description }}</p>
                            </div>

                            @if ($item->remarks)
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Staff
                                        Remarks</label>
                                    <p
                                        class="text-gray-600 italic bg-amber-50/50 p-3 rounded-lg border border-amber-100/50">
                                        "{{ $item->remarks }}"</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-12 pt-6 border-t border-gray-50 flex items-center justify-between">
                            <p class="text-xs text-gray-400 italic">Last modification:
