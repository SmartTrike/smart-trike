            ...
            <div class="mb-8 bg-indigo-900 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold mt-1">{{ $activeFare->label }}</h2>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400">Description Label</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400 text-right">Trip Fare</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400 text-right">Terminal</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400 text-right">Hire Fare</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400 text-center">Status</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-400">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($fares as $fare)
                                <tr class="hover:bg-gray-50/30 transition-colors {{ $fare->is_current ? 'bg-indigo-50/20' : '' }}">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-lg {{ $fare->is_current ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400' }} flex items-center justify-center border border-gray-100">
                                                <x-tabler-coin class="h-5 w-5" />
                                            </div>
                                            <div>
                                                <div class="text-sm font-bold text-gray-900">{{ $fare->label }}</div>
                                                <div class="text-[10px] text-gray-400 uppercase tracking-tighter">ID: #{{ str_pad($fare->id, 4, '0', STR_PAD_LEFT) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @else
                                            <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-400">
                                                Inactive
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center gap-4">
                                            ...
                                            @if(!$fare->is_current)
                                                <form action="{{ route('admin.fare.update', $fare->id) }}" method="POST" class="inline">
                                                    @csrf @method('PUT')
                                                    <input type="hidden" name="is_current" value="1">
                                                    <button type="submit" class="text-gray-900 font-bold hover:text-indigo-600 transition-colors">Activate</button>
                                                </form>
                                            @endif
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">Create your first pricing rule to get started.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
