...
<div class="min-h-screen bg-gray-50/30 p-4 lg:p-10">
    <div class="max-w-4xl mx-auto">
        <header class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Edit Fare: {{ $fare->label }}</h1>
            <p class="text-gray-500 mt-1">Modify pricing details or change the active status of this fare description.</p>
        </header>

        <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
            {{-- Use PUT method for updates --}}
            <form action="{{ route('admin.fare.update', $fare->id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="label" value="{{ old('label', $fare->label) }}"
                    ...
                    <div class="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 {{ $fare->is_current ? 'bg-green-100 text-green-600' : 'bg-indigo-100 text-indigo-600' }} rounded-lg flex items-center justify-center">
                                    @if($fare->is_current)
                                        <x-tabler-check class="w-6 h-6" />
                                    @else
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">
                                        @if($fare->is_current)
                                            This is currently the live pricing structure.
                                        @else
                                            Enabling this will replace the current active fare.
                                        @endif
                                    </p>
                                </div>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_current" value="1" class="sr-only peer" {{ old('is_current', $fare->is_current) ? 'checked' : '' }}>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-50">
                        <div class="flex items-center gap-2 text-gray-400">
                            <x-tabler-history class="h-5 w-5" />
                            <span class="text-xs font-medium">Last updated: {{ $fare->updated_at->format('M d, Y h:i A') }}</span>
                        </div>

                        <div class="flex gap-4 items-center">
                            <a href="{{ route('admin.fare.index') }}" class="px-6 py-3 text-sm font-bold text-gray-500 hover:text-gray-700">Cancel Changes</a>
